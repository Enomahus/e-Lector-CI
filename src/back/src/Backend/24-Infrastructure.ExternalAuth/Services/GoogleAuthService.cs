...
namespace Infrastructure.ExternalAuth.Services
{
    public class GoogleAuthService : IExternalAuthService
    {
        ...
        public GoogleAuthService(
            IOptionsFactory<ExternalAuthConfiguration> configurationFactory,
            IOptions<AppConfiguration> appConfiguration
        )
        {
            var config = configurationFactory.Create(ExternalAuthServiceKeys.GoogleConfiguration);
            if(config.ClientSecret is null)
            {
                throw new ConfigurationMissingException("Missing configuration : GoogleAuth.ClientSecret");
            }
            if (config.ClientId is null)
            {
                throw new ConfigurationMissingException("Missing configuration : GoogleAuth.ClientId");
            }

            if (appConfiguration.Value.AppUrl is null)
            {
            ...
            _redirectUri = $"{appConfiguration.Value.AppUrl}/login/google";
        }
        ...
        )
        {
            ...
            var request = peopleService.People.Get("people/me?personFields=names,emailAddresses,phoneNumbers");
