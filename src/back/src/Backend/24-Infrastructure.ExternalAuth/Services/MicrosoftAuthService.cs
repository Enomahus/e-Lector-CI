...
namespace Infrastructure.ExternalAuth.Services
{
    public class MicrosoftAuthService : IExternalAuthService
    {
        ...
        )
        {
            ...
            {
                throw new ConfigurationMissingException("Missing configuration : MicrosoftAuth.ClientSecret");
            }
            if (config.ClientId is null)
            {
                throw new ConfigurationMissingException("Missing configuration : MicrosoftAuth.ClientId");
            }
        ...
        )
        {
            ...
            var result = await graphClient.Me.GetAsync(
                config =>
                {
                    config.QueryParameters.Select = ["givenName", "surname", "mail", "mobilePhone"];
                },
                cancellationToken
            );
            ...
            )
            {
                if(AccessToken is null)
                {
                    ...
                    var authResult = await clientApp
                        ...
                        ["openid","profile", "offline_access","User.Read"],_code)
