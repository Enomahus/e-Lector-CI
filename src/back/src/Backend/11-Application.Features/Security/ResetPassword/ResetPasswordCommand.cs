...
namespace Application.Features.Security.ResetPassword
{
    [AuditParameters(
        ...
        Action = nameof(AuditAction.PasswordResetMailSent)
    )]
    ...
    public class ResetPasswordCommandValidator : AbstractValidator<ResetPasswordCommand>
    {
        public ResetPasswordCommandValidator(ReadOnlyDbContext context)
        {
            RuleFor(c => c.UserEmail)
                ...
                {
                    RuleFor(c => c.UserEmail)
                        .MustAsync((email, token) => context.Users.AnyAsync(u => u.Email == email, token))
                        .WithMessage(ValidationErrorCode.UserMustExist.ToString());
                });

            RuleFor(c => c.ResetToken).NotEmpty().WithMessage(ValidationErrorCode.Required.ToString());
       ...
       : IRequestHandler<ResetPasswordCommand, Result>
    {
        public async Task<Result> Handle(ResetPasswordCommand request, CancellationToken cancellationToken)
        {
            ...
            var user =
                await context.Users.FirstOrDefaultAsync(u => u.Email == request.UserEmail, cancellationToken)
            ...
            try
            {
                result = await userManager.ResetPasswordAsync(user, request.ResetToken!, request.Password!);
            }
            catch (Exception e)
            {
