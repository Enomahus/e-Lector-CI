...
) : SeederBase(context, userManager)
{
    ...
    private async Task SeedConstituenciesAsync()
    {
        ...
        {
            var dbConstituency = await _context.Constituencies.FirstOrDefaultAsync(c =>
                c.Wording == constituency.Wording && c.Level == constituency.Level
            );
    ...
    private async Task SeedRolesAsync()
    {
        ...
        var allPermissions = await _context.AppPermissions.ToDictionaryAsync(p => p.PermissionCode);
        ...
        foreach (var seed in RolesData.RolesSeed)
        {
            ...
            var targetActions = allActions.Where(a => seed.Value.Contains(a.ActionCode)).ToHashSet();
