...
namespace Web.Services
{
    ...
    ) : CurrentUserTokenService<Guid, long>(httpContextAccessor, tokenRoleClaimBuilder, permissionsService),
        ICurrentUserService,
...

    {
        ...
        public string? UserEmail => _httpContextAccessor
        ...
        public string? LanguageCode => _httpContextAccessor
        ...
        public Guid? ImpersonatorId =>
            Guid.TryParse(
                _httpContextAccessor
                    .HttpContext?.User.Claims.FirstOrDefault(c => c.Type == AppConstants.ImpersonatorIdClaim)
                    ?.Value ?? "",
                out var parsedId
            )
                ? parsedId
        ...
        public string? ImpersonatorEmail =>
            _httpContextAccessor
                .HttpContext?.User.Claims.FirstOrDefault(c => c.Type == AppConstants.ImpersonatorEmailClaim)
                ?.Value;
        ...
        )
        {
            ...
            var userRolePermissions = await GetRolePermissionsAsync(tokenRoles.UserRoles, cancellationToken);
            // Check for global admin permissions
            var userAdminPermissions = userRolePermissions.Where(r => r == nameof(AppPermission.SuperAdmin));
            ...
            return userRolePermissions.Intersect(entityRoles).Union(userAdminPermissions).Distinct();
        }
    }
}
