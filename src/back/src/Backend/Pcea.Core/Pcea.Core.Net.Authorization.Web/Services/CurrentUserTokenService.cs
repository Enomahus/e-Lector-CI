...
namespace Pcea.Core.Net.Authorization.Web.Services
{
        ...
        : ICurrentUserPermissionsProvider, ICurrentUserEntityPermissionsProvider<T_EntityId>
    {
        ...
        )
        {
            ...
            var rolesPermissions = await GetRolePermissionsAsync(tokenRoles.UserRoles, cancellationToken);
            return rolesPermissions;
        }

        public abstract Task<bool> IsCurrentUserAuthenticatedAsync(CancellationToken cancellationToken = default);
    ...
    )
        {
            ...
            var userRolesPermissions = await GetRolePermissionsAsync(tokenRoles.UserRoles, cancellationToken);
            if (!userEntityRoles.Any())
            {
                // If user has no roles on entity, then use its global roles
                return userRolesPermissions;
            }
            var entityRolesPermissions = await GetRolePermissionsAsync(userEntityRoles, cancellationToken);
