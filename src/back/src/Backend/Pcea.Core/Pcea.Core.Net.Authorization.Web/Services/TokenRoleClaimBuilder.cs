...
namespace Pcea.Core.Net.Authorization.Web.Services
{
        ...
        where T_EntityId : notnull
    {
        ...
        protected virtual string GetEntityRoleToClaimValue(T_EntityId entityId, IEnumerable<string> roles)
        {
            return $"{entityId}_{string.Join(';', roles)}";
        }

        protected virtual (T_EntityId entityId, IEnumerable<string> roles) GetEntityRoleFromClaimValue(
            string claimValue
        )
        {
