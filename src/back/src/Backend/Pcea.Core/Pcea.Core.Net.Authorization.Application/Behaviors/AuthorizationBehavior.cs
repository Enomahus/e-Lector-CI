...
public class AuthorizationBehavior<TRequest, TResponse>(ICurrentUserPermissionsProvider currentUserPermissionsProvider,
    ...
    : Behavior<TRequest,TResponse> where TRequest : notnull
{
    ...
    )
    {
        ...
        if (
            !await currentUserPermissionsProvider.IsCurrentUserAuthenticatedAsync(cancellationToken)
        )
        {
        ...
        // Check permissions
        var permissionCodes = await currentUserPermissionsProvider.GetCurrentUserPermissionsAsync(
            cancellationToken
        );
        var requiredPermissionsCodes = withPermissionAttributes
            .SelectMany(a => a.UserPermissions)
            .Distinct();
